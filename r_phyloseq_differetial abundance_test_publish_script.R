...
############################ create relationship matrix of otus for upset plot   
...
input_data = read.csv("/home/hl46161/publish_living_mulch/exported-feature-table-for-deseq2/family_otu_table.tsv",sep = "\t",check.names = FALSE)
### make otu name to be rowname
row.names(input_data) = input_data$OTU_ID
input_data$OTU_ID = NULL

#### remove zero value
input_data[is.na(input_data)] <- 0
#input_data <- mutate_all(input_data, function(x) as.numeric(as.character(x)))

#######read in input data 
input_metadata = read.csv("/home/hl46161/publish_living_mulch/living_mulch_data_metafile_massalin_no_blank.tsv",sep = "\t")
#########subset to contain only may and june samples 
row.names(input_metadata) = input_metadata$SampleID
input_metadata$SampleID = NULL
input_metadata$treatment_modified = factor(input_metadata$treatment,
                                           levels = c("NoCover", "CerealRye", "CrimsonClover","LivingMulch"))
input_metadata$Date_taken_modified = factor(input_metadata$Date_taken,
                                            levels = c("2018-05-21", "2018-06-28", "2018-08-31"))
##############################run linear mix model on family level taxa and soil chemical variable
fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_N',
  fixed_effects = c('N'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_TOC',
  fixed_effects = c('TOC'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_pH',
  fixed_effects = c('pH'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_Amon',
  fixed_effects = c('Amon'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_BS',
  fixed_effects = c('BS'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_CEC',
  fixed_effects = c('CEC'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_Ca',
  fixed_effects = c('Ca'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_Mg',
  fixed_effects = c('Mg'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_P',
  fixed_effects = c('P'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_LBCEQ',
  fixed_effects = c('LBCEQ'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

fit_data <- Maaslin2(
  input_data, input_metadata, '/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_N_LBCEQ',
  fixed_effects = c('LBCEQ','N'),random_effects = c('replicate','date_taken'),
  standardize = TRUE,plot_scatter = TRUE,min_prevalence = 0.6)

#################################

#extract results from N and LBCEQ and link them with family taxa table 
family_taxa_table$feature <- rownames(family_taxa_table)

significant_result_N <- read.csv("/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_N/significant_results.tsv",sep="\t")
significant_result_N  <- dplyr::left_join(significant_result_N,family_taxa_table,by="feature")
significant_result_N  <- significant_result_N[significant_result_N$qval <= 0.05,]


significant_result_LBCEQ  <- read.csv("/home/hl46161/publish_living_mulch/massalin2_result/family_OTU_soil_data_aassociation_60_filter_LBCEQ/significant_results.tsv",sep="\t")
significant_result_LBCEQ  <- dplyr::left_join(significant_result_LBCEQ,family_taxa_table,by="feature")
significant_result_LBCEQ <- significant_result_LBCEQ[significant_result_LBCEQ$qval <= 0.05,]

LBCEQ_results <- significant_result_LBCEQ %>%
  mutate(association =
           case_when(coef <= 0 ~ "-",
                     coef > 0 ~ "+",
           ))

N_results <- significant_result_N %>%
  mutate(association =
           case_when(coef <= 0 ~ "-",
                     coef > 0 ~ "+",
           ))
...
#
LBCEQ_results <- LBCEQ_results[LBCEQ_results$N.not.0 >= 21,]
LBCEQ_results <- LBCEQ_results %>% dplyr::select(metadata,feature,association)
#LBCEQ_results <- sapply(LBCEQ_results, as.character)
#LBCEQ_results[is.na(LBCEQ_results)] <- " "

N_results <- N_results[N_results$N.not.0 >= 21,]
N_results <- N_results %>% dplyr::select(metadata,feature,association)

#join N and LBCEQ value into one 
association_results <- dplyr::full_join(LBCEQ_results,N_results,by="feature")
association_results$metadata <- paste(association_results$metadata.x,association_results$metadata.y)
association_results$metadata <- gsub("NA", "", association_results$metadata)
association_results <- association_results %>% dplyr::select(metadata,feature,association.x,association.y)
colnames(association_results) <-c("metadata","feature","Correlation_LBCEQ","Correlation_N")

# join the N and LBCEQ results with difference abudant taxa 
lm_cr_cc_otu_table_association <-  dplyr::left_join(lm_cr_cc_otu_table,association_results,by = c("OTU_ID" = "feature"))
lm_cr_cc_otu_table_association$OTU_ID <- NULL
lm_cr_cc_otu_table_association <- lm_cr_cc_otu_table_association %>% dplyr::select(Taxa,Abundance,metadata,Correlation_LBCEQ,Correlation_N)
lm_cr_cc_otu_table_association <- lm_cr_cc_otu_table_association[order(lm_cr_cc_otu_table_association$Abundance,decreasing = TRUE),]
#lm_cr_cc_otu_table_association <- sapply(lm_cr_cc_otu_table_association, as.character)
#lm_cr_cc_otu_table_association[is.na(lm_cr_cc_otu_table_association)] <- " " 

#replave the prefix with full taxonomic level
lm_cr_cc_otu_table_association$Taxa <- gsub("f_", "Family",lm_cr_cc_otu_table_association$Taxa)
lm_cr_cc_otu_table_association$Taxa <- gsub("o_", "Order",lm_cr_cc_otu_table_association$Taxa)
lm_cr_cc_otu_table_association$Taxa <- gsub("p_", "Phylum",lm_cr_cc_otu_table_association$Taxa)
lm_cr_cc_otu_table_association$Taxa <- gsub("c_", "Class",lm_cr_cc_otu_table_association$Taxa)
lm_cr_cc_otu_table_association$Taxa <- gsub(" ", "",lm_cr_cc_otu_table_association$Taxa)
lm_cr_cc_otu_table_association <- lm_cr_cc_otu_table_association %>%  separate(Taxa,c("Rank","Taxa"))

lm_cr_cc_otu_table_association[lm_cr_cc_otu_table_association$Correlation_N =="+"]

#
write.table(lm_cr_cc_otu_table_association,"/home/hl46161/publish_living_mulch/massalin2_result/lm_cr_cc_otu_table_association.tsv",sep='\t',row.names = FALSE,na="")


#
lm_vs_cc_otu_table_association <-  dplyr::left_join(lm_vs_cc_otu,association_results,by = c("OTU_ID" = "feature"))
lm_vs_cc_otu_table_association$OTU_ID <- NULL
lm_vs_cc_otu_table_association <- lm_vs_cc_otu_table_association %>% dplyr::select(Taxa,Abundance,metadata,Correlation_LBCEQ,Correlation_N)
lm_vs_cc_otu_table_association <- lm_vs_cc_otu_table_association[order(lm_vs_cc_otu_table_association$Abundance,decreasing = TRUE),]
lm_vs_cc_otu_table_association$Abundance <- round(lm_vs_cc_otu_table_association$Abundance, digits = 4)
lm_vs_cc_otu_table_association$Taxa <- gsub("f_", "Family",lm_vs_cc_otu_table_association$Taxa)
lm_vs_cc_otu_table_association$Taxa <- gsub("o_", "Order",lm_vs_cc_otu_table_association$Taxa)
lm_vs_cc_otu_table_association$Taxa <- gsub("p_", "Phylum",lm_vs_cc_otu_table_association$Taxa)
lm_vs_cc_otu_table_association$Taxa <- gsub("c_", "Class",lm_vs_cc_otu_table_association$Taxa)
lm_vs_cc_otu_table_association$Taxa <- gsub(" ", "",lm_vs_cc_otu_table_association$Taxa)
lm_vs_cc_otu_table_association <- lm_vs_cc_otu_table_association %>%  separate(Taxa,c("Rank","Taxa"))

lm_vs_cc_otu_table_association 
write.table(lm_vs_cc_otu_table_association,"/home/hl46161/publish_living_mulch/massalin2_result/lm_cc_otu_table_association.tsv",sep='\t',row.names = FALSE,na="")


lm_vs_cr_otu_table_association <-  dplyr::left_join(lm_vs_cr_otu,association_results,by = c("OTU_ID" = "feature"))
lm_vs_cr_otu_table_association$OTU_ID <- NULL
lm_vs_cr_otu_table_association <- lm_vs_cr_otu_table_association %>% dplyr::select(Taxa,Abundance,metadata,Correlation_LBCEQ,Correlation_N)
lm_vs_cr_otu_table_association <- lm_vs_cr_otu_table_association[order(lm_vs_cr_otu_table_association$Abundance,decreasing = TRUE),]
lm_vs_cr_otu_table_association$Abundance <- round(lm_vs_cr_otu_table_association$Abundance, digits = 4)
lm_vs_cr_otu_table_association$Taxa <- gsub("f_", "Family",lm_vs_cr_otu_table_association$Taxa)
lm_vs_cr_otu_table_association$Taxa <- gsub("o_", "Order",lm_vs_cr_otu_table_association$Taxa)
lm_vs_cr_otu_table_association$Taxa <- gsub("p_", "Phylum",lm_vs_cr_otu_table_association$Taxa)
lm_vs_cr_otu_table_association$Taxa <- gsub("c_", "Class",lm_vs_cr_otu_table_association$Taxa)
lm_vs_cr_otu_table_association$Taxa <- gsub(" ", "",lm_vs_cr_otu_table_association$Taxa)
lm_vs_cr_otu_table_association <- lm_vs_cr_otu_table_association %>%  separate(Taxa,c("Rank","Taxa"))
lm_vs_cr_otu_table_association

write.table(lm_vs_cr_otu_table_association,"/home/hl46161/publish_living_mulch/massalin2_result/lm_cr_otu_table_association.tsv",sep='\t',row.names = FALSE,na="")
